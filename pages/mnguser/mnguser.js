...
Page({
  ...
  getMyusers: function () {
    ...
    util.request(api.GetAlluserBypage, search).then(res => {
      if (!res.data.err) {
  ...
  addMyUser: function(e) {
    const form = {
      name: e.currentTarget.dataset.name,
      weixin_openid: e.currentTarget.dataset.openid
    }
    const index = e.currentTarget.dataset.index;
    util.request(api.GetMyusers, form, 'POST').then(res => {
      if (res.data.err) {
        $Message({
          content: res.data.err,
          type: 'error'
      ...
      this.data.userList[index].addflag = true;
      this.setData({
